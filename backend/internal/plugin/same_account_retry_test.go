...
func TestSameAccountRetryOnSoleAccount(t *testing.T) {
	tests := []struct {
		...
		{name: "upstream 504 is not retried on the same account", first: transientInjected(504, "HTTP 504: gateway timeout"), secondOK: true, wantCalls: 1, wantStatus: 504},
		{name: "plugin watchdog timeout is not retried", first: transientInjected(502, "上游首字节或流停滞超时（插件守卫断开）: context canceled"), secondOK: true, wantCalls: 1, wantStatus: 502},
...
func TestSameAccountRetryable(t *testing.T) {
	cases := []struct {
		...
		{"504", forwardExecution{outcome: transientInjected(504, "gateway timeout")}, false},
		{"watchdog timeout wording", forwardExecution{outcome: transientInjected(502, "上游首字节或流停滞超时（插件守卫断开）")}, false},
		{"english timeout wording", forwardExecution{outcome: transientInjected(502, "i/o timeout")}, false},
